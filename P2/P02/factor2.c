...
{
  int i,j,n,nf,f[16],m[16],nDivisores; // the product of the first 16 primes is larger than 2^64
  nDivisores= 1;
  ...
  for(i = 1;i < argc;i++){
   ...
   for(int i=0;i <=16;i++){
    ...
    for(int i=0;i <=nDivisores;i++){
      printf("Divisor %d: %d",i,d[i]);
    } 
  }
  return 0;
}
