...
#define max                         decoder_global_data.max
...
{                                                                                                                             //
  ...
  for(int i = 0 ; i < _c_->n_symbols ; i++){                                                                                  // itera sobre as codificações
    int j;                                                                                                                    //
    if(_c_->data[i].codeword[0] == _encoded_message_[encoded_idx])
      possibilities[decoded_idx]++;
    ...
    if(_c_->data[i].codeword[j] == '\0' && _number_of_solutions_ != 1L){                                                      // chegasse ao final da codificação com tds os bit iguais e ainda n se tem soluções
      ...
      recursive_decoder(encoded_idx + j,decoded_idx + 1, good_decoded_size, possibilities);                                                  // aceitar como a codificação encontrada nesta iteração como certa e ir para o próximo
    }                                                                                                                         //
  }                                                                                                                           //
}                                                                                                                             //
...
void try_it(code_t *c,int message_size,int show_results)
{
  ...
  int * possibilities = (int *)malloc((size_t)_max_decoded_message_size_ * sizeof(int));
  ...
  recursive_decoder(0,0,0,possibilities);
