  ...
  if(code_decoded == 0){                                                                                                      // caso n nos falte codificar nenhum
    for(int i = 0 ; i < _c_->n_symbols ; i++){                                                                                // iteração por todas as codificações
      if(_encoded_message_[encoded_idx] == _c_->data[i].codeword[0]){                                                         // procura-se uma codificação k tenha o inicio semelhante ao bit atualmente recebido
        ...
        if((decoded_idx + 1 - good_decoded_size) > _max_extra_symbols_)                                                       // caso o numero de simbolos extras nesta iteração seja maior ao max até agr
          _max_extra_symbols_ = ((decoded_idx+1) - good_decoded_size);                                                        // define-se o max até agr de simbolos extras como o obtido agr
        recursive_decoder(encoded_idx + 1, decoded_idx + 1, good_decoded_size, strlen(_c_->data[i].codeword) - 1);            // chamamento da função para se confirmar o próximo bit
      }                                                                                                                       //
    }                                                                                                                         //
...
{                                                                                                                             //
  ...
  // Verificação dos bits dentro da codificaçao a se avaliar
    ...
    int len = 0;                                                                                                              // length da ultima codificação aceite
    while(_c_->data[_decoded_message_[decoded_idx - 1]].codeword[len] != '\0')                                                // calculo do len
      len++;                                                                                                                  // incremento
    code_decoded = len - code_decoded;                                                                                        // index do bit onde estamos em relação à codificação atualmente a ser avaliada
...
{                                                                                                                             //
  ...
  if(code_decoded == 0){                                                                                                      // caso n nos falte codificar nenhum
    for(int i = 0 ; i < _c_->n_symbols ; i++){                                                                                // iteração por todas as codificações
      if(_encoded_message_[encoded_idx] == _c_->data[sorted[i]].codeword[0] && _number_of_solutions_ != 1L){                  // procura-se uma codificação k tenha o inicio semelhante ao bit atualmente recebido
        _decoded_message_[decoded_idx] = sorted[i];                                                                           // simbolo k se inicia com o bit recebido
